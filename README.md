...
> 수익률 계산: 월별 종가 기준 (1개월=2포인트, 3개월=4포인트, 12개월=13포인트)

---

### 성장성 (20점)

연간 매출 YoY 성장률 기준 (우선순위: 연간 데이터 → 분기 YoY 비교)
...
> 상승여력 = (컨센서스 평균 목표주가 − 현재가) / 현재가 × 100

**매수 비율 (최대 7점)** — strongBuy + buy 합산 / 전체 추천 수
...
| 총점 | 방향 | 신뢰도 |
|------|------|--------|
| 60점 이상 | **매수 (buy)** | 총점 ÷ 100 |
| 40점 미만 | **매도 (sell)** | (100 − 총점) ÷ 100 |
| 40 ~ 59점 | **중립 (hold)** | 0.50 (고정) |
...
| 항목 | 산정 방식 |
|------|---------|
| **목표가** | LLM이 밸류에이션 모델(PER·DCF·PBR·EV/EBITDA) 적용 후 기본 시나리오 값 |
| **손절가** | LLM 설정, 비관 시나리오 기준. 프롬프트 가이드라인: 현재가 대비 약 −15~−25% 수준 |
