        ...
        for col, sig in zip(cols, row_sigs):
            with col:
                ...
                m1.metric("신뢰도", f"{sig['confidence']:.1%}",
                    help="퀀트·LLM 신호가 일치하면 ×1.15 상향, 불일치하면 HOLD로 0.35 고정")
                if sig["quant_score"] is not None:
                    m2.metric("퀀트", f"{sig['quant_score']}/100",
                        help="밸류에이션(20)+퀄리티(25)+모멘텀(20)+재무건전성(20)+성장성(15) 합산\n≥60 매수 · 40~59 중립 · <40 매도")
                ...
                if sig["target_price"]:
                    p2.metric(
                        ...
                        help=(
                            "재무 데이터(EPS·EV/EBITDA·FCF 등)를 기반으로 AI가 산출한 12개월 기준 적정 주가입니다.\n"
                            "PER·DCF·EV/EBITDA 중 해당 종목에 적합한 방법론을 적용하며, 낙관·기본·비관 시나리오 중 기본값을 표시합니다.\n"
                            "월가 애널리스트 컨센서스(평균·최고·최저)와 비교·검증되며, 괴리가 클 경우 자동 보정됩니다."
                        ),
                    )
                if sig["stop_loss"]:
                    p3.metric(
                        ...
                        help=(
                            "투자 논거가 훼손될 경우 손실을 제한하기 위한 하방 기준가입니다.\n"
                            "14일 평균 실질 변동폭(ATR)의 2배를 현재가에서 차감하여 산출합니다.\n"
                            "변동성이 큰 종목은 손절 폭이 넓고, 안정적인 종목은 좁게 설정됩니다."
                        ),
                    )
                    ...
                    with st.expander("퀀트 세부 점수"):
                        ...
                        _QUANT_HELP = {
                            ...
                            "재무건전성": (
                                ...
                                "FCF/순이익 비율 (5점): Piotroski 발생액 신호\n"
                                "  FCF>0→+3점, FCF≥순이익×80%→+2점(이익의 질 우수)"
                            ),
                            "성장성": (
                                "만점 15점\n"
                                "매출 YoY (7점): 재무제표 기반\n"
                                "  >25%→7점 / >15%→5점 / >7%→3점 / >0%→1점\n"
                                "EPS 성장률 (5점): yfinance earningsGrowth\n"
                                "  >25%→5점 / >10%→3점 / >0%→1점\n"
                                "애널리스트 목표주가 상승여력 (3점)\n"
                                "  >20%→3점 / >10%→2점 / >0%→1점"
                            ),
        ...
        if sig_path.exists():
            ...
            if sig.get("target_price"):
                st.metric(
                    "목표가", f"${sig['target_price']:,.1f}",
                    help=(
                        "애널리스트가 재무 데이터를 기반으로 산출한 12개월 기준 적정 주가입니다.\n"
                        "PER·DCF·PBR·EV/EBITDA 등 밸류에이션 모델 중 해당 종목에 적합한 방법론을 적용하며,\n"
                        "낙관·기본·비관 3가지 시나리오 중 기본 시나리오 값을 표시합니다."
                    ),
                )
            if sig.get("stop_loss"):
                st.metric(
                    ...
                    help=(
                        "투자 논거가 훼손될 경우 손실을 제한하기 위한 하방 기준가입니다.\n"
                        "비관 시나리오를 기준으로 현재가 대비 약 15~25% 하락 수준에서 설정됩니다."
                    ),
                )
